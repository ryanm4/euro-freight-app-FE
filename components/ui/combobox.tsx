...
function Combobox({
  ...
  // Filter items based on the search query input
  const filteredItems = React.useMemo(() => {
    const selectedItem = items.find((item) => String(item.value) === String(value))
    const isSelectedLabel = selectedItem && selectedItem.label === inputValue
    if (!inputValue || isSelectedLabel) return items
    return items.filter((item) =>
      item.label.toLowerCase().includes(inputValue.toLowerCase())
    )
  }, [items, value, inputValue])
  ...
  return (
    <ComboboxPrimitive.Root
      ...
      onInputValueChange={(newVal) => setInputValue(newVal)}
    >
      <ComboboxInput
